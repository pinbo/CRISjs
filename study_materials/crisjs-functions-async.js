...
async function processFile(file) {
    try {
        ...
        let result = arrayBufferToString(arrayBuffer);
        return result;
    } catch (err) {
        console.log(err);
    }
...
async function readfasta(){
	...
	var f = files[0];
    //readFile(f, getfasta);
    let result = await processFile(f);
    getfasta(result);
}
...
async function getSeq() {
    ...
    await readfasta();
	console.log(seqdict);
	var sequences = document.getElementById('sequences').innerHTML.split(/ +/);
	...
	var template = seqdict[geneID].toUpperCase();
...
//add example input
function putExample(){
    document.getElementById("sequences").innerHTML = "DELLA-4B-Kronos GAGGAGGTGGACGAGC GCGCCGCGCCCGACG GCTGGCGGCGCTCGGGTACA";
}